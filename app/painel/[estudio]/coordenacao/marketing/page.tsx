...
export default async function MarketingPage({
  ...
  // Dia e Semana navegam por mês; Mês e Ano navegam por ano.
  ...
  if (protegida && !desbloqueada) {
    return (
      <PortaSenha
        titulo="Coordenação"
        estudioSlug={slug}
        destino={`/painel/${slug}/coordenacao/marketing`}
  ...
  const { data: leadsData, error: erroLeads } = await supabase
    .from('leads')
    .select('estado, entrada, visita_data, visita_marcada_em, walk_in')
    .eq('estudio_id', estudio.id)

  if (erroLeads) {
    throw new Error(erroLeads.message)
  }
  const leads = (leadsData ?? []) as LeadMarketing[]
  ...
  const toggleVista = (
    <div className="mt-4 flex flex-wrap gap-1.5">
      {VISTAS.map((v) => (
        <Link
          key={v}
          href={`/painel/${slug}/coordenacao/marketing?vista=${v}`}
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      ...
      <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
        {estudio.nome} · funil de leads ao longo do tempo.
      </p>
      ...
      {vista === 'dia' && (
        <>
          <div className="mt-4 flex items-center justify-center gap-3">
            <Link
              href={`/painel/${slug}/coordenacao/marketing?vista=dia&mes=${mesAnteriorParam}&metrica=${metricaAtual}`}
            ...
            <Link
              href={`/painel/${slug}/coordenacao/marketing?vista=dia&mes=${mesSeguinteParam}&metrica=${metricaAtual}`}
              className="rounded-md border border-black/10 px-3 py-1.5 text-sm dark:border-white/10"
            >
              →
            </Link>
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {METRICAS_MARKETING.map((m) => (
              <Link
                key={m.chave}
                href={`/painel/${slug}/coordenacao/marketing?vista=dia&mes=${mesAtualParam}&metrica=${m.chave}`}
          ...
          {(() => {
            ...
            return (
              <div className="mt-4 rounded-xl border border-black/10 bg-white p-4 dark:border-white/10 dark:bg-zinc-950">
      ...
      {vista === 'semana' && (
        <>
          <div className="mt-4 flex items-center justify-center gap-3">
            <Link
              href={`/painel/${slug}/coordenacao/marketing?vista=semana&mes=${mesAnteriorParam}`}
              className="rounded-md border border-black/10 px-3 py-1.5 text-sm dark:border-white/10"
            >
              ←
            </Link>
            ...
            <Link
              href={`/painel/${slug}/coordenacao/marketing?vista=semana&mes=${mesSeguinteParam}`}
              className="rounded-md border border-black/10 px-3 py-1.5 text-sm dark:border-white/10"
            >
              →
            </Link>
      ...
      {vista === 'mes' && (
        <>
          <div className="mt-4 flex items-center justify-center gap-3">
            <Link
              href={`/painel/${slug}/coordenacao/marketing?vista=mes&ano=${anoSelecionado - 1}`}
              className="rounded-md border border-black/10 px-3 py-1.5 text-sm dark:border-white/10"
            >
              ←
            </Link>
      ...
      {vista === 'ano' && (
        <>
          <div className="mt-4 flex items-center justify-center gap-3">
            <Link
              href={`/painel/${slug}/coordenacao/marketing?vista=ano&ano=${anoSelecionado - 1}`}
              className="rounded-md border border-black/10 px-3 py-1.5 text-sm dark:border-white/10"
            >
              ←
            </Link>
